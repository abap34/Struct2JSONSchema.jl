...
"""
    serialize_number_for_default(value) -> Union{Int, Float64, Nothing}

Normalize Julia numeric types to JSON-compatible Int or Float64 for use as default values.
Returns `nothing` if the value is not a number.
"""
...
"""
    serialize_datetime_for_default(value) -> Union{String, Nothing}

Serialize date/time types to ISO 8601 string format for use as default values.
Returns `nothing` if the value is not a recognized date/time type.
"""
...
"""
    serialize_stringlike_for_default(value) -> Union{String, Nothing}

Serialize types that have a natural string representation for use as default values.
Returns `nothing` if the value is not string-convertible.
"""
...
"""
    serialize_array_for_default(value, field_type::Type) -> Union{Vector, Nothing}

Recursively serialize array values to JSON-compatible format for use as default values.
Returns `nothing` if serialization fails for any element.
"""
function serialize_array_for_default(value, field_type::Type)::Union{Vector, Nothing}
    ...
    serialized = [default_serialize_for_schema(elem_type, item) for item in value]
...
"""
    serialize_dict_for_default(value, field_type::Type) -> Union{Dict{String, Any}, Nothing}

Recursively serialize dictionary values to JSON-compatible format for use as default values.
Keys are converted to strings, values are recursively serialized.
Returns `nothing` if serialization fails for any value.
"""
function serialize_dict_for_default(value, field_type::Type)::Union{Dict{String, Any}, Nothing}
    ...
    for (k, v) in value
        serialized = default_serialize_for_schema(val_type, v)
...
"""
    default_serialize_for_schema(field_type::Type, value) -> Union{RepresentableValue, Nothing}
...
`register_default_serializer!`, `register_type_serializer!`, or `register_field_serializer!`.
...
# Examples
```julia
default_serialize_for_schema(DateTime, DateTime(2024, 1, 1))  # "2024-01-01T00:00:00"
default_serialize_for_schema(Int, 42)  # 42
default_serialize_for_schema(Function, () -> nothing)  # nothing (unsupported)
```
"""
function default_serialize_for_schema(field_type::Type, value)::Union{RepresentableValue, Nothing}
...
"""
    with_field_context(f::Function, ctx::SchemaContext, T::Type, field::Symbol)

Execute function `f` with field context temporarily set for serialization.
Restores original context on completion, even if an exception occurs.
"""
...
"""
    try_custom_serializer(serializer::Function, field_type::Type, value, ctx::SchemaContext) -> Any

Attempt to execute a single custom serializer function with error handling.
Returns the serialized value if successful, `nothing` if it fails or returns nothing.
Exceptions are caught and logged in verbose mode.
"""
...
"""
    serialize_default_value(ctx::SchemaContext, field_type::Type, value) -> Any

Internal function that orchestrates default value serialization.
Tries registered custom serializers in FIFO order, then falls back to
`default_serialize_for_schema`. Returns SERIALIZATION_FAILED sentinel if
serialization fails, otherwise returns the JSON-compatible value.
"""
function serialize_default_value(
    ctx::SchemaContext,
    field_type::Type,
    value
)
    ...
    result = default_serialize_for_schema(field_type, value)
...
"""
    process_field_default!(ctx::SchemaContext, T::Type, field::Symbol, field_type::Type, value) -> Nothing

Process and register default value for a single struct field.
"""
function process_field_default!(
    ...
    with_field_context(ctx, T, field) do
        json_value = serialize_default_value(ctx, field_type, value)
...
"""
    validate_instance_type(instance) -> Type

Validate that instance is suitable for default value registration.
Returns the type of the instance if valid, throws ArgumentError otherwise.
"""
...
"""
    register_defaults!(ctx::SchemaContext, instance) -> Nothing
...
ctx = SchemaContext()
register_defaults!(ctx, ServerConfig("localhost", 8080))
...
- Use `register_type_serializer!` or `register_field_serializer!` for custom serialization
"""
function register_defaults!(ctx::SchemaContext, instance)::Nothing
...
"""
    register_default_serializer!(serializer::Function, ctx::SchemaContext) -> Nothing
...
```
"""
function register_default_serializer!(serializer::Function, ctx::SchemaContext)::Nothing
    push!(ctx.default_serializers, serializer)
    return nothing
end

"""
    register_type_serializer!(serializer::Function, ctx::SchemaContext, T::Type) -> Nothing

Register a custom serializer for all values of a specific type.
...
ctx = SchemaContext()
register_type_serializer!(ctx, Color) do value, ctx
...
```
"""
function register_type_serializer!(serializer::Function, ctx::SchemaContext, T::Type)::Nothing
    register_default_serializer!(ctx) do field_type, value, ctx
        if typeof(value) === T
...
"""
    register_field_serializer!(serializer::Function, ctx::SchemaContext, Parent::Type, field::Symbol) -> Nothing

Register a custom serializer for a specific field of a struct.
...
register_field_serializer!(ctx, Metrics, :created_at) do value, ctx
    Int(datetime2unix(value))
end
```
"""
function register_field_serializer!(
    serializer::Function,
    ctx::SchemaContext,
    Parent::Type,
    field::Symbol
