using Test
using Struct2JSONSchema: SchemaContext, generate_schema, register_defaults!, k, register_field_override!, register_type_override!, register_field_description!, register_field_serializer!, register_type_serializer!, register_default_serializer!, treat_union_nothing_as_optional!, register_abstract!, register_skip_fields!, RepresentableScalar
using Dates
import Base: UUID
...
@testset "Complex - nested structs with custom serializers and defaults" begin
    ...
    register_type_serializer!(ctx, Point) do value, ctx
        [value.x, value.y]
    end
...
@testset "Complex - multiple field serializers with nested types" begin
    ...
    register_field_serializer!(ctx, LogEntry, :id) do value, ctx
        uppercase(string(value))
    end

    # created は Unix タイムスタンプとして
    register_field_serializer!(ctx, LogEntry, :created) do value, ctx
        Int(datetime2unix(value))
    end
...
@testset "Complex - deeply nested structures with mixed serialization" begin
    ...
    register_type_serializer!(ctx, Address) do value, ctx
    ...
    register_type_serializer!(ctx, Contact) do value, ctx
        parts = String[]
...
@testset "Complex - enum with defaults" begin
    ...
    # Enum 型のカスタムシリアライザー
    register_type_serializer!(ctx, Status) do value, ctx
        string(value)
    end
...
@testset "Complex - tuple and namedtuple with defaults" begin
    ...
    register_field_serializer!(ctx, Coordinate, :point) do value, ctx
        [value[1], value[2]]
    end

    register_field_serializer!(ctx, Coordinate, :meta) do value, ctx
...
@testset "Complex - multiple type overrides with defaults" begin
    ...
    # カスタムシリアライザー
    register_type_serializer!(ctx, Point2D) do value, ctx
        [value.x, value.y]
    end

    register_type_serializer!(ctx, Point3D) do value, ctx
...
@testset "Complex - versionnumber and rational defaults" begin
    ...
    register_type_serializer!(ctx, Rational{Int}) do value, ctx
        Float64(value)
    end
...
@testset "Complex - all features combined" begin
    ...
    register_field_serializer!(ctx, Stock, :purchased) do value, ctx
