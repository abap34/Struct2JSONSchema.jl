...
# Backward compatible version (defaults to "type_not_representable")
function record_unknown!(ctx::SchemaContext, T; message::Union{Nothing, String} = nothing)
    record_unknown!(ctx, T, "type_not_representable"; message = message)
end

# Extended version with reason
...
is_unknown_type(ctx::SchemaContext, T) = any(entry.type === T for entry in ctx.unknowns)
